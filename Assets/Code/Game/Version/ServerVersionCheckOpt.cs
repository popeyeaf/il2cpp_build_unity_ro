...
namespace RO
{
	public class ServerVersionCheckOpt : VersionUpdateOperation
	{
		...
		protected override void StartCheck()
		{
			...
			_cdnAddress = "http://43.228.85.251/res/" + ApplicationHelper.platformFolder + "/";
		...
		void TryGetRemoteClientVersion()
		{
			...
			if (data.IsArray)
			{
				for (int i = 0; i < data.Count; i++)
				{
					...
					HttpWWWRequestOrder order = new HttpWWWRequestOrder(url + ":90/versionan2.php?", _timeOut, operation, false, true);
			...
			_manager.ShowYesConfirm(content, ROWords.GOTO, () => {
				...
				if (infos != null && infos.Count > 0)
				{
					...
					if (string.IsNullOrEmpty(appUrl))
					{
						...
						Application.OpenURL("http://43.228.85.251/?dir=RO" + chinese);
					}
					else
					{
