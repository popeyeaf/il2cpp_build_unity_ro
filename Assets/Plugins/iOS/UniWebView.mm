...
@interface UniWebViewManager()<WKNavigationDelegate> {
...
-(void) addManagedWebViewName:(NSString *)name insets:(UIEdgeInsets)insets {
    ...
    UniWebView *webView = [[UniWebView alloc] initWithFrame:unityView.frame configuration:configuration];
...
- (void)webViewNameCleanCache:(NSString *)name {
    WKWebView *webView = _webViewDic[name];
    ...
    if (webView) {
        ...
        [dataStore removeDataOfTypes:dataTypes modifiedSince:dateFrom completionHandler:^{
            NSLog(@"Cache cleaned for web view: %@", name);
        }];
    }
}
...
- (void)webViewName:(NSString *)name setZoomEnable:(BOOL)enable {
    ...
    if (webView) {
        webView.scrollView.bouncesZoom = enable;
    }
}
    ...
    if (webView) {
        [webView evaluateJavaScript:js completionHandler:^(id result, NSError *error) {
            if (callback) {
                ...
                UnitySendMessage([name UTF8String], "EvalJavaScriptFinished", [resultString UTF8String]);
            }
        }];
    }
}
...
- (void)webView:(UniWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    UIView *spinnerView = [webView viewWithTag:100];
    [spinnerView removeFromSuperview];
    
    NSString *webViewName = [self webViewName:webView];
    //[webView updateToolBtn];
    
    [webView evaluateJavaScript:@"document.URL" completionHandler:^(id result, NSError *error) {
        if (result && [result isKindOfClass:[NSString class]]) {
            NSString *currentURL = (NSString *)result;
            UnitySendMessage([webViewName UTF8String], "LoadComplete", [currentURL UTF8String]);
        }
    }];
}

- (void)webView:(UniWebView *)webView didFailNavigation:(WKNavigation *)navigation withError:(NSError *)error {
    UIView *spinnerView = [webView viewWithTag:100];
    [spinnerView removeFromSuperview];
   ...
   // [webView updateToolBtn];
    ...
    [webView evaluateJavaScript:@"document.URL" completionHandler:^(id result, NSError *error) {
        if (result && [result isKindOfClass:[NSString class]]) {
            ...
            UnitySendMessage([webViewName UTF8String], "LoadComplete", [errorMessage UTF8String]);
        }
    }];
}
...
- (BOOL)checkURLScheme:(NSURL *)url withSchemes:(NSArray<NSString *> *)schemes {
...
- (BOOL)webView:(UniWebView *)webView shouldStartLoadWithNavigationAction:(WKNavigationAction *)navigationAction {
    NSURL *url = navigationAction.request.URL;
    WKNavigationType navigationType = navigationAction.navigationType;
    
    NSArray<NSString *> *allowedSchemes = @[@"http", @"https"]; // Add your allowed schemes here
    
    BOOL canResponse = [self checkURLScheme:url withSchemes:allowedSchemes];
    
    if (canResponse) {
        NSString *webViewName = [self webViewName:webView];
        NSString *rawMessage = [NSString stringWithFormat:@"%@", url];
        UnitySendMessage([webViewName UTF8String], "ReceivedMessage", [rawMessage UTF8String]);
        return NO;
    } else {
        // Check if openLinkInExternalBrowser is true
        if (navigationType == WKNavigationTypeLinkActivated && webView.openLinkInExternalBrowser) {
            [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:nil];
            return NO;
        } else {
            NSString *webViewName = [self webViewName:webView];
            UnitySendMessage([webViewName UTF8String], "LoadBegin", [url.absoluteString UTF8String]);
        }
    }
