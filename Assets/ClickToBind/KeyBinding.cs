...
public class KeyBinding : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
	...
	public UILabel keyDisplay;

	//Used for color changing during key binding
	public GameObject button;
	public UIButton keyButton;
    ...
    void Awake()
	{
		if (keyButton == null)
		{
			keyButton = button.GetComponent<UIButton>();
		}

		// ตรวจสอบว่า keyButton มีค่าหรือไม่ และเพิ่ม onClick
		if (keyButton != null)
		{
			EventDelegate.Set(keyButton.onClick, () => ChangeKeyCode(true));
		}
    ...
    IEnumerator Wait()
    {
		// ถอดการทำงานที่เกี่ยวข้องกับ keyButton.onClick
		keyButton.onClick.Clear();

		yield return new WaitUntil(() => !Input.GetMouseButton(0));

		// เพิ่มการทำงานใหม่ใน keyButton.onClick
		EventDelegate.Set(keyButton.onClick, () => ChangeKeyCode(true));
	}
