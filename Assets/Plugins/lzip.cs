...
public class lzip
{
	...
	#if (UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN)
    [DllImport(libname, EntryPoint = "zipEX2"
...
//compressed file sizes
	public static int zipFiles, zipFolders;
// global integers that store the number of files and folders in a zip file.
	...
	public static long getFileInfo (string zipArchive, string path, byte[] FileBuffer = null)
	{
		...
		if (res == -1) { /*Debug.Log("Input file not found.");*/
			return -1;
		}
		...
		#else
		if (res == -2) { /*Debug.Log("Path does not exist: " + path);*/
			return -2;
		}
		#endif
		if (res == -3) { /*Debug.Log("Entry info error.");*/
			return -3;
		}
		...
		if (!File.Exists (logPath)) { /*Debug.Log("Info file not found.");*/
			return -4;
		}
	...
	public static int compressBuffer (byte[] source, int length, ref byte[] outBuffer, int bufferOffset, int levelOfCompression)
	{
		if (levelOfCompression < 0)
			levelOfCompression = 0;
		if (levelOfCompression > 10)
		...
		ptr = zipCompressBuffer (sbuf.AddrOfPinnedObject (), length, levelOfCompression, ref siz);
		...
		Marshal.Copy (ptr, outBuffer, bufferOffset, siz);
	...
	public static int decompressBuffer(byte[] source, int length, ref byte[] outBuffer, int bufferOffset)
	{
		...
		if (siz == 0 || ptr == IntPtr.Zero) { sbuf.Free(); releaseBuffer(ptr); return -1; }
	...
	public static int replace_entry (string zipArchive, string arc_filename, byte[] newFileBuffer, int level = 9)
	{
		...
		if (buffer2File (level, zipArchive, arc_filename, newFileBuffer, true))
			return 1;
		else
			return -6;
	...
	public static int extract_entry (string zipArchive, string arc_filename, string outpath, byte[] FileBuffer = null, int[] proc = null)
	{
		...
		if (FileBuffer != null) {
			GCHandle fbuf = GCHandle.Alloc (FileBuffer, GCHandleType.Pinned);
			if (proc != null)
				res = zipEntry (null, arc_filename, outpath, fbuf.AddrOfPinnedObject (), FileBuffer.Length, pbuf.AddrOfPinnedObject ());
			else
		...
		if (proc != null)
			res = zipEntry (@zipArchive, arc_filename, outpath, IntPtr.Zero, 0, pbuf.AddrOfPinnedObject ());
		else
			res = zipEntry (@zipArchive, arc_filename, outpath, IntPtr.Zero, 0, IntPtr.Zero);
		pbuf.Free ();
		return res;
	...
	public static int decompress_File (string zipArchive, string outPath, int[] progress, byte[] FileBuffer = null, int[] proc = null)
	{
		...
		if (FileBuffer != null) {
			...
			if (proc != null)
				res = zipEX (null, @outPath, ibuf.AddrOfPinnedObject (), fbuf.AddrOfPinnedObject (), FileBuffer.Length, pbuf.AddrOfPinnedObject ());
			else
				res = zipEX (null, @outPath, ibuf.AddrOfPinnedObject (), fbuf.AddrOfPinnedObject (), FileBuffer.Length, IntPtr.Zero);
			#else
		...
		if (proc != null)
			...
			res = zipEX (@zipArchive, @outPath, ibuf.AddrOfPinnedObject (), IntPtr.Zero, 0, IntPtr.Zero);
		ibuf.Free ();
		pbuf.Free ();
		return res;
	}
